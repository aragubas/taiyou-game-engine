...
def Initialize(reg_dir,IsSystemReg=True):
    ...
    for x in temp_reg_keys:
        ...
        file = open(x,"r")
        if IsSystemReg:
            system_reg_contents.append(file.read().rstrip().replace("%N","\n"))
            system_reg_keys.append(CorrectKeyName)
        else:
...
# -- System Keys -- #
def ReadSystemKey(keyName):
    return system_reg_contents[system_reg_keys.index(keyName)]


def ReadSystemKey_int(keyName):
    return int(system_reg_contents[system_reg_keys.index(keyName)])

def ReadSystemKey_float(keyName):
    return float(system_reg_contents[system_reg_keys.index(keyName)])


def ReadSystemKey_bool(keyName):
    return bool(system_reg_contents[system_reg_keys.index(keyName)])
