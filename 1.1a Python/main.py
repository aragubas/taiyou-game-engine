...
def main():
    ...
    while True:
        ...
        if GameError:
            # -- Display the Taiyou Text --
            font = pygame.font.SysFont('Ubuntu', 28, True, False)
            font = pygame.font.SysFont('Ubuntu', 10, True, False)

            text = font.render('Taiyou Game Engine has stopped worked.', True, (255, 255, 255))
            text2 = font3.render('ReportedError: ' + GameErrorText, True, (255, 255, 255))

            DISPLAY.blit(text, (5, 5))  # -- Render the Taiyou Game Engine text
            DISPLAY.blit(text2, (5, 35))  # -- Render the current vars state
        ...
        try:
            ...
            print("An error has stopped the Game from working.\n" + str(ex))
            IsGameRunning = False
            GameDrawAllowed = False
            pygame.mouse.set_visible(True)
            GameError = True
            GameErrorText = str(ex)
