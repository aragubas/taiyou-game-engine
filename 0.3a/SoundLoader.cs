...
namespace TaiyouGameEngine.Desktop
{
    public class SoundLoader
    {
        ...
        public static void FindAllSounds(string SourceFolder)
        {
            // First, we need to list all files on SPRITES directory
            string[] AllSounds = Directory.GetFiles(SourceFolder + "/SOUND/", "*.wav*", SearchOption.AllDirectories);
            ...
            foreach (var file in AllSounds)
            {
                ...
                SoundFiltedName = SoundFiltedName.Replace(SourceFolder + "/SOUND/", "");
                ...
                if (SoundID == -1)
                {
                    if (info.Extension == ".wav")
                    {
                        ...
                        AllLoadedSounds_Content.Add(LoadSoundFromFile(SourceFolder + "/SOUND/" + SoundFiltedName));
