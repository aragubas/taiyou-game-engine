...
class AnimationController:
    def __init__(self, multiplierSpeed=1.0, maxValue=255, minValue=0, multiplierRestart=False):
        """
        Controls the flux of an Animation
        :param multiplierSpeed: Animation Speed
        :param maxValue: Maximum Value
        :param minValue: Minimum Value
        :param multiplierRestart: Restart the multiplier when animation reach's the first stage
        """
        self.Enabled = True
...
def LimitValueRange(Input, MinValue, MaxValue):
    """
    ...
    :return:
    """
    if int(Input) < MinValue:  # -- R
        Input = MinValue

    if int(Input) > MaxValue:  # -- R
        Input = MaxValue
